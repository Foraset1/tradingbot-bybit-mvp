...
Для одиночного исследовательского snapshot до завершения воспроизводимого backtest нельзя
удалять:
...
проверки Parquet и повторного совпадения raw SHA-256. Отдельная apply-команда требует
сохранённый dry-run, точный fingerprint и ещё раз проверяет весь список до удаления. Этот
процесс описан в [`DAILY_ARCHIVE.md`](DAILY_ARCHIVE.md).
