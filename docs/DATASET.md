...
Текущая итерация строит канонический рыночный слой. Признаки, decision grid и labels
`TP_FIRST` / `SL_FIRST` / `TIMEOUT` будут строиться поверх него отдельно, чтобы raw
нормализация не смешивалась с параметрами исследовательского эксперимента.
