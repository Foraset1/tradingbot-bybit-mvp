...
описан отдельно в [`HISTORICAL_BOOTSTRAP.md`](HISTORICAL_BOOTSTRAP.md). Он не заменяет
live-сбор стакана и не синтезирует отсутствующие microstructure-поля.
...
```bash
sudo env TRADINGBOT_COLLECTOR_MEMORY=6g TRADINGBOT_COLLECTOR_CPUS=2.0 \
...
использует до 500 000 равномерных по времени строк, LightGBM — полный fit-набор.
