...
символов во всех дневных datasets. Обычный `build-research` принимает только дни со
статусом `clean`. Каталоги с явными `kline_gap` предназначены для continuity-aware V3,
который исключает отдельные небезопасные окна; этот builder такой гарантии не даёт.
