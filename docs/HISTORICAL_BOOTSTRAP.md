...
Профиль `price_futures_v1` импортирует бесплатную официальную историю Bybit. Он дополняет,
но не заменяет live-сборщик.
...
запускаются вслепую: live raw/archive продолжают расти, а retention требует отдельного
проверенного dry-run и ручного применения по [`DAILY_ARCHIVE.md`](DAILY_ARCHIVE.md). При
приближении к 15 GiB свободного места импорт остановится безопасно.
